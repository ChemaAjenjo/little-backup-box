...
# If there is a telegram.conf file in the root of the storage device
# move telegram.conf from the card to /telegram-little-backup-box.conf
if [ -f "$STORAGE_MOUNT_POINT/telegram.conf" ]; then
  mv "$STORAGE_MOUNT_POINT/telegram.conf" /etc/telegram-little-backup-box.conf
fi
...
# If we have internet connection, charge telegram configurations
if [ $NETWORK -eq 1 ];then
  source /etc/telegram-little-backup-box.conf
fi
